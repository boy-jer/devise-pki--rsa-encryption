...
*IMPORTANT:* This code is incomplete, and does not have any test framework to validate the security that it is supposed to supply. Till I have time to write coverage tests to verify that there is no plaintext leakage for each of the ORM's this code should not be put in any production environment. It is currently under heavy development and the ActiveRecord support is incomplete, however I am using it in a mongoid rails-3.1 application successfully. If you think this module is going to be useful to you send me a message and I will try and prioritize the features you need. 
